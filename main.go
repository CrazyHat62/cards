...
func main() {
	...
	dk.LoadStandardDeckSprites()
	defer dk.UnloadStandardDeckSprites()

	var currentScreen gameScreen
	var currentGame game
	currentScreenENUM := LOGO

	FrameRec = rl.NewRectangle(0, 0, dk.SpriteSizeW, dk.SpriteSizeH)
	FrameRecDest = rl.NewRectangle(0, 0, float32(dk.SpriteSizeW/2), float32(dk.SpriteSizeH/2))
...
func getCardSource(card dk.Card, frameRec rl.Rectangle) rl.Rectangle {
	...
	source := frameRec
	source.X = float32(dk.SpriteSizeW*ix + 1)
	source.Y = float32(dk.SpriteSizeH * iy)
...
func getInitialGridDest(idx int, frameRec rl.Rectangle) rl.Rectangle {
	...
	xgrd := idx % 8
	ygrd := idx / 8
	dest.X = float32(dk.SpriteSizeW*xgrd + 10*(xgrd+1) + 90)
	dest.Y = float32(dk.SpriteSizeH + (ygrd)*50 + 40)
	return dest
}
